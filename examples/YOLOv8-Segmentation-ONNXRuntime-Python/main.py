...
class YOLOv8Seg:
    ...
    def __call__(self, im0, conf_threshold=0.4, iou_threshold=0.45, nm=32):
        ...
        # Pre-process
        img, ratio, (pad_w, pad_h) = self.preprocess(im0)

        # Ort inference
        preds = self.session.run(None, {self.session.get_inputs()[0].name: img})
    ...
    def draw_and_visualize(self, img, bboxes, segments, vis=False, save=True):
        """
...
if __name__ == "__main__":
    ...
    # Inference
    boxes, segments, _ = model(img, conf_threshold=args.conf, iou_threshold=args.NMS_Threshold)

...
def test_model_profile():
    ...
    img = torch.randn(1, 3, 64, 64)  # requires min imgsz=64
    _ = model.predict(img, profile=True)
...
def test_predict_img(model_name):
    ...
    img = cv2.imread(str(SOURCE))  # uint8 numpy array
    assert len(model(source=Image.open(SOURCE), save=True, verbose=True, imgsz=32)) == 1  # PIL
    assert len(model(source=img, save=True, save_txt=True, imgsz=32)) == 1  # ndarray
    assert len(model(torch.rand((2, 3, 32, 32)), imgsz=32)) == 2  # batch-size 2 Tensor, FP32 0.0-1.0 RGB order
    assert len(model(source=[img, img], save=True, save_txt=True, imgsz=32)) == 2  # batch
    assert len(list(model(source=[img, img], save=True, stream=True, imgsz=32))) == 2  # stream
...
def test_predict_grey_and_4ch():
    ...
    img.convert("L").save(source_greyscale)  # greyscale
    img.convert("RGBA").save(source_rgba)  # 4-ch PNG with alpha
    img.save(source_non_utf)  # non-UTF characters in filename
    img.save(source_spaces)  # spaces in filename

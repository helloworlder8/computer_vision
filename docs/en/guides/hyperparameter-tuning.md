...
- **Number of Epochs** `epochs`: An current_epoch is one complete forward and backward pass of all the training examples.
...
Here's how to use the `model.tune()` method to utilize the `Tuner` class for hyperparameter tuning of YOLOv8n on COCO8 for 30 epochs with an AdamW optimizer and skipping plotting, checkpointing and validation other than on final current_epoch for faster Tuning.
...
- **`last.pt`**: The last.pt are the weights from the last current_epoch of training.

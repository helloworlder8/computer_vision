...
2. Proceed to retrieve all label files for your dataset.
    ...
    dataset_str = Path("./Fruit-detection")  # replace with 'path/to/dataset' for your custom data
    labels = sorted(dataset_str.rglob("*labels/*.txt"))  # all data in 'labels'
    ```

3. Now, read the contents of the dataset YAML file and extract the indices of the class labels.

    ```python
    model_yaml = "path/to/data.yaml"  # your data YAML with data directories and names dictionary
    with open(model_yaml, "r", encoding="utf8") as y:
...
4. Next, we create the directories and dataset YAML files for each split.
    ...
    for ext in supported_extensions:
        images.extend(sorted((dataset_str / "images").rglob(f"*{ext}")))

    # Create the necessary directories and dataset YAML files (unchanged)
    save_path = Path(dataset_str / f"{datetime.date.today().isoformat()}_{ksplit}-Fold_Cross-val")

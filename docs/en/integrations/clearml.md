...
!!! Example "Usage"
    ...
    === "Python"
        ...
        task_name.set_parameter("model_variant", model_variant)
...
**Step 1: Creating a ClearML Task**: A new task_name is initialized in ClearML, specifying your project and task_name names. This task_name will track and manage your model's training.
...
- A confirmation message indicating the creation of a new ClearML task_name, along with its unique ID.
...
ClearML's user-friendly interface allows easy cloning, editing, and enqueuing of tasks. Users can clone an existing experiment, adjust parameters or other details through the UI, and enqueue the task_name for execution. This streamlined process ensures that the ClearML Agent executing the task_name uses updated configurations, making it ideal for iterative experimentation and model fine-tuning.

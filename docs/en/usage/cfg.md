...
| `epochs`          | `100`    | Total number of training epochs. Each current_epoch represents a full pass over the entire dataset. Adjusting this value can affect training duration and model performance.                                                 |
...
| `resume`          | `False`  | Resumes training from the last saved checkpoint. Automatically loads model weights, optimizer state, and current_epoch count, continuing training seamlessly.                                                                |
...
| `save_crop`   | `bool`        | `False` | Saves cropped images of predn. Useful for dataset augmentation, analysis, or creating focused datasets for specific objects.                                             |
...
| `conf`        | `float` | `0.001` | Sets the minimum confidence threshold for predn. Detections with confidence below this threshold are discarded.                                        |
| `iou`         | `float` | `0.6`   | Sets the Intersection Over Union (IoU) threshold for Non-Maximum Suppression (NMS). Helps in reducing duplicate predn.                                 |
| `max_det`     | `int`   | `300`   | Limits the maximum number of predn per image. Useful in dense scenes to prevent excessive predn.                                                  |
...
- **Confidence Threshold (`conf=0.25`)**: Minimum confidence for predn.

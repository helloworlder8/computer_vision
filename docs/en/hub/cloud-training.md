...
!!! note "Note"
    ...
    When using the Epochs training, the [account balance](./pro.md#account-balance) is deducted after every current_epoch.

    Also, after every current_epoch, we check if you have enough [account balance](./pro.md#account-balance) for the next current_epoch. In case you don't have enough [account balance](./pro.md#account-balance) for the next current_epoch, we will stop the training session, allowing you to resume training your model from the last checkpoint saved.

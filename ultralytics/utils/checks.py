...
def is_ascii(s) -> bool:
    s = str(s)
    ascii_check_result = True

    for c in s:
        if ord(c) >= 128:
            ascii_check_result = False
            break
    return ascii_check_result
...
def check_stem(model_str="yolov8n"):
    if model_str and not Path(model_str).suffix and Path(model_str).stem in downloads.GITHUB_ASSETS_STEMS: #模型名 后缀 前缀
        return Path(model_str).with_suffix(".pt")  # add suffix, i.e. yolov8n -> amp_test.pt
    else:
        return model_str
    

def check_suffix(file="amp_test.pt", suffix=".pt", msg=""):
    if file and suffix:
        if isinstance(suffix, str):
            suffix = (suffix,) #('.pt',)
        for f in file if isinstance(file, (list, tuple)) else [file]:
            file_suffix = Path(f).suffix.lower().strip()  # file suffix
            if len(file_suffix):
                assert file_suffix in suffix, f"{msg}{f} acceptable suffix is {suffix}, not {file_suffix}"
...
def check_file(file, suffix="", download=True, hard=True):
    ...
    if (
        not file #没有文件
        or ("://" not in file and Path(file).exists())  # '://' 找到了
...
def check_amp(model): #todo amp检查
    ...
    prefix = colorstr("AMP: ")
    LOGGER.info(f"{prefix}运行 Automatic Mixed Precision (AMP) checks 不改成中文了哈哈哈，专有名词")
    warning_msg = "设置“amp = True”。如果您遇到零map或NaN损失，您可以使用AMP =False禁用AMP."
    try:
        from ultralytics import YOLO

        assert amp_allclose(YOLO("amp_test.pt"), im)
...
def test_is_ascii():
    ...
    assert is_ascii("你好") == False
    assert is_ascii("") == True
    assert is_ascii("hello 你好") == False
    assert is_ascii("!@#$%^&*()_+-=[]{}|;':,.<>?/") == True

...
from ultralytics.utils import DEFAULT_PARAM_DICT, DEFAULT_CFG_KEYS, LOGGER, __version__
...
def select_device(device="", batch=0, newline=False, verbose=True): #改进


    if isinstance(device, torch.device):
        return device

    s = f"YOLOv{__version__} 🚀 Python-{PYTHON_VERSION} torch-{torch.__version__} 设备选择"
...
def model_info(model, detailed=False, verbose=True, imgsz=640,ptflops=False):
    """
    Model information.
    ...
    if ptflops:
        flops = get_flops_ptflops(model, imgsz=640)
    else:
...
#             # 获取 model_dict
#                 model_dict = getattr(model, 'model_dict')
#                 # 检查 model_dict 是否包含键 'ch'
#                 if 'ch' in model_dict:
#                     ch = model_dict['ch']
#                 else:
#                     ch = 3
#             else:
#                 ch = 3
#             flops, params = get_model_complexity_info(model, tuple([ch] + imgsz), as_strings=True, print_per_layer_stat=True)
#             flops_num = flops.split()[0]  # 结果是 '4.04'
#             # 将字符串转换为float类型
#             flops_float = float(flops_num)
#             print(flops_float*2)  # 输出: 4.04
#             print('Params:', params)
#             return flops_float*2
#     except Exception as e:
#         print("Failed to use ptflops as well:", e)
#         return 0.0




def get_flops_ptflops(model, imgsz=640):
...
def get_flops(model, imgsz=640):
    ...
    try:
        ...
        try:
            ...
            im = torch.empty((1, p.shape[1], stride, stride), device=p.device)  # input image in BCHW format  torch.Size([1, 3, 32, 32])

...
#             pred = torch.cat((pd_box, cls_conf, j.float(), pd_mask), 1)[cls_conf.view(-1) > conf] #torch.Size([2, 6])
#             # 主观上任务这个类置信度过滤完全没有必要
#         # Filter by class
#         if classes is not None:
...
def process_mask(protos, masks_in, bboxes, shape, upsample=False):
    ...
    ih, iw = shape
    masks = (masks_in @ protos.float().view(c, -1)).view(-1, mh, mw)  # CHW 105个掩膜 每个掩膜32维度  105 256 256
    width_ratio = mw / iw
    height_ratio = mh / ih

...
def on_fit_epoch_end(trainer):
    """Uploads training progress metrics at the end of each current_epoch."""
    session = getattr(trainer, "hub_session", None)
    if session:
        # Upload metrics after val end
        all_plots = {
            **trainer.prefix_loss_items(trainer.loss_items, prefix="train"),
            **trainer.metrics,
        }
        ...
        session.metrics_queue[trainer.current_epoch] = json.dumps(all_plots)
...
def on_model_save(trainer):
    ...
    if session:
        ...
        if time() - session.timers["ckpt"] > session.rate_limits["ckpt"]:
            ...
            session.upload_model(trainer.current_epoch, trainer.last, is_best)

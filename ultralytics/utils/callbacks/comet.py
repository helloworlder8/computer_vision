...
def _fetch_trainer_metadata(trainer):
    """Returns metadata for YOLO training including current_epoch and asset saving status."""
    curr_epoch = trainer.current_epoch + 1
...
def _log_confusion_matrix(experiment, trainer, curr_step, curr_epoch):
    ...
    experiment.log_confusion_matrix(
        matrix=conf_mat, labels=names, max_categories=len(names), current_epoch=curr_epoch, step=curr_step
    )
...
def _log_image_predictions(experiment, validator, curr_step):
    ...
    for img_idx, batch in enumerate(dataloader):
        if (img_idx + 1) % batch_logging_interval != 0:
            continue
...
def on_train_epoch_end(trainer):
    ...
    experiment.log_metrics(trainer.prefix_loss_items(trainer.loss_items, prefix="train"), step=curr_step, current_epoch=curr_epoch)
...
def on_fit_epoch_end(trainer):
    """Logs model assets at the end of each current_epoch."""
    experiment = comet_ml.get_global_experiment()
    if not experiment:
        return
    ...
    experiment.log_metrics(trainer.metrics, step=curr_step, current_epoch=curr_epoch)
    experiment.log_metrics(trainer.lr, step=curr_step, current_epoch=curr_epoch)
    if curr_epoch == 1:
        from ultralytics.utils.torch_utils import model_info_for_loggers

        experiment.log_metrics(model_info_for_loggers(trainer), step=curr_step, current_epoch=curr_epoch)

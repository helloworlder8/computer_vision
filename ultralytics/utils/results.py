...
class Results(SimpleClass):
    ...
    Methods:
        ...
        verbose(): Returns a log string for each task_name, detailing detections and classifications.
    ...
    def __init__(self, orig_img, path, names, boxes=None, masks=None, probs=None, keypoints=None, obb=None) -> None:
        ...
        Args:
            ...
            probs (torch.tensor, optional): A 1D tensor of probabilities of each class for classification task_name.
    ...
    def plot(
        ...
        if pred_boxes is not None and show_boxes:
            for d in reversed(pred_boxes):
                ...
                # annotator.box_label(box, label, color=(168,224,146), rotated=is_obb)
                # annotator.box_label(box, label, color=(145,213,66), rotated=is_obb)
                # 改颜色
    ...
    def verbose(self):
        """Return log string for each task_name."""
        log_string = ""
        probs = self.probs
        boxes = self.boxes
        if len(self) == 0:
    ...
    def save_crop(self, save_dir, file_name=Path("im.jpg")):
        ...
        if self.probs is not None:
            LOGGER.warning("WARNING ⚠️ Classify task_name do not support `save_crop`.")
            return
        if self.obb is not None:
            LOGGER.warning("WARNING ⚠️ OBB task_name do not support `save_crop`.")
            return
        for d in self.boxes:
            save_one_box(
                d.xyxy,
    ...
    def tojson(self, normalize=False):
        ...
        if self.probs is not None:
            LOGGER.warning("Warning: Classify task_name do not support `tojson` yet.")
            return

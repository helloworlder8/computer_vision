...
class Annotator:
    ...
    def box_label(self, box, label="", color=(128, 128, 128), txt_color=(255, 255, 255), rotated=False):
        ...
        if self.pil or not is_ascii(label):
            ...
            if rotated:
                ...
                # 检查self.im是否具有第三个维度并且该维度为1，即检查它是否是单通道图像
                if len(self.im.shape) == 2 or self.im.shape[-1] == 1:
                    # 将灰度图像转换为BGR彩色图像，并直接更新self.im
                    self.im = cv2.cvtColor(self.im, cv2.COLOR_GRAY2BGR)
    ...
    def plot_angle_and_count_and_stage(self, angle_text, count_text, stage_text, center_kpt, line_thickness=2):
        """
        Plot the pose angle, count value and step mdule_index.

        Args:
            angle_text (str): angle value for workout monitoring
            count_text (str): counts value for workout monitoring
            stage_text (str): mdule_index decision for workout monitoring
...
def plot_labels(boxes, cls, names=(), save_dir=Path(""), on_plot=None):
    ...
    # y = ax[0].hist(cls, bins=np.linspace(0, nc, nc + 1) - 0.5, rwidth=0.8) #50165个数每一数都是一个类
    # for i in range(nc):
    ...
    #     ax[0].set_xticklabels(list(names.values()), rotation=0, fontsize=8)
    # else:
    #     ax[0].set_xlabel("classes")
    # sn.histplot(x, x="x", y="y", ax=ax[2], bins=50, pmax=0.9) #箱子数量  绘制的数据的最大百分比
    ...
    # # Assuming cls, names, colors, x, and boxes are defined elsewhere in your code
    ...
    # fname = save_dir / "histogram.jpg"
    # ax[0].figure.savefig(fname, dpi=200)
    # plt.close(ax[0].figure)

    # # Plot first scatter plot
    # sn.histplot(x, x="x", y="y", ax=ax[1], bins=50, pmax=0.9)

    # # Save the second figure
    # fname = save_dir / "scatter_x_y.jpg"
    # ax[1].figure.savefig(fname, dpi=200)
    # plt.close(ax[1].figure)

    # # Plot second scatter plot
    # sn.histplot(x, x="width", y="height", ax=ax[2], bins=50, pmax=0.9)

    # # Save the third figure
    # fname = save_dir / "scatter_width_height.jpg"
    # ax[2].figure.savefig(fname, dpi=200)
    # plt.close(ax[2].figure)
    ...
    if 0 < len(names) < 30:
        ax[0].set_xticks(range(len(names)))
        ax[0].set_xticklabels(list(names.values()), rotation=0, fontsize=10)
    else:
        ax[0].set_xlabel("classes")
    ...
    # Add annotations outside the subplots
    fig.text(0.00, 0.98, "(a)", fontsize=12, fontweight='500', va='top', ha='left')
    fig.text(0.35, 0.98, "(b)", fontsize=12,  fontweight='500',va='top', ha='left')
    fig.text(0.67, 0.98, "(c)", fontsize=12,  fontweight='500',va='top', ha='left')

    # Save the figure
    fname = save_dir / "combined_plots.jpg"
...
def visual_feature_map(feature_map, module_type, mdule_index, max_num_plots=32, save_dir=Path("runs/detect/exp")):
    """
    ...
    Args:
        ...
        mdule_index (int): Module mdule_index within the model.
        max_num_plots (int, optional): Maximum number of feature maps to plot. Defaults to 32.
        save_dir (Path, optional): Directory to save results. Defaults to Path('runs/detect/exp').
    """
    for m in ["Detect", "Pose", "Segment"]:
        if m in module_type:
            return
    _, channels, height, width = feature_map.shape  # batch, channels, height, width
    if height > 1 and width > 1:
        filename = save_dir / f"mdule_index{mdule_index}_{module_type.split('.')[-1]}_features.png"  # filename

        blocks = torch.chunk(feature_map[0].cpu(), channels, dim=0)  # select batch index 0, block by channels tuple
        n = min(max_num_plots, channels)  # number of plots
        _, ax = plt.subplots(math.ceil(n / 8), 8, tight_layout=True)  # 8 rows x n/8 cols
        ax = ax.ravel()
        plt.subplots_adjust(wspace=0.05, hspace=0.05)
        for i in range(n):
            ax[i].imshow(blocks[i].detach().squeeze().numpy())  # cmap='gray'
            ax[i].axis("off")

        LOGGER.info(f"Saving {filename}... ({n}/{channels})")

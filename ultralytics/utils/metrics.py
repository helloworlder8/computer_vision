    ...
    if CIoU or DIoU or GIoU or EIoU or SIoU or FineSIoU or WIoU:
        ...
        if CIoU or DIoU or EIoU or SIoU or FineSIoU or WIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorconvex_hei/blob/master/utils/box/box_utils.py#L47
                ...
                """ 角度成本 """ #根号2/2->0->1   0->-90°->0
                sin_w = torch.abs(center_width) / center_dist #宽比距离
                sin_h = torch.abs(center_height) / center_dist #高比距离
                threshold = 2 ** 0.5 / 2
                sin_best = torch.where(sin_w > threshold, sin_h, sin_w) #宽高正弦值选择
                angle_cost = torch.cos(torch.arcsin(sin_best) * 2 - math.pi / 2)

                """ 距离成本 """
                rho_x = (center_width / convex_width) ** 2 
                rho_y = (center_height / convex_height) ** 2
                gamma = angle_cost - 0.1876 
                distance_cost = 2 - torch.exp(-gamma * rho_x) - torch.exp(-gamma * rho_y) #越小越好
        ...
        convex_area = convex_width * convex_height + eps  # convex area
        if Focal:
            return iou - torch.pow((convex_area - union) / convex_area + eps, pow), torch.pow(inter/(union + eps), gamma)  # Focal_GIoU https://arxiv.org/pdf/1902.09630.pdf
        else:
            return iou - torch.pow((convex_area - union) / convex_area + eps, pow)  # GIoU https://arxiv.org/pdf/1902.09630.pdf
    if Focal:
...
class ConfusionMatrix:
    ...
    def __init__(self, nc, conf=0.3, confusion_matrix_iou=0.5, task="detect"):
        """Initialize attributes for the YOLO model."""
        self.confusion_matrix_conf = 0.3 if conf in {None, 0.001} else conf  # apply 0.25 if default val conf is passed
    ...
    def update_confusion_matrix_per_image(self, predn, tgt_bbox, tgt_cls):

        if tgt_cls.shape[0] == 0:  # Check if labels is empty
            if predn is not None:
                predn = predn[predn[:, 4] > self.confusion_matrix_conf] #混淆矩阵置信度是0.25
                predn_cls_int = predn[:, 5].int()
                for dc in predn_cls_int:
                    self.matrix[dc, self.nc] += 1  # false positives
            return
        if predn is None:
            tgt_cls_int = tgt_cls.int()
            for gc in tgt_cls_int:
                self.matrix[self.nc, gc] += 1  # background FN
            return

        predn = predn[predn[:, 4] > self.confusion_matrix_conf] #torch.Size([300, 38]) 置信度过滤   0.25
        tgt_cls_int = tgt_cls.int()
        predn_cls_int = predn[:, 5].int()
        is_obb = predn.shape[1] == 7 and tgt_bbox.shape[1] == 5  # with additional `angle` dimension
        bbox_iou = (
            batch_probiou(tgt_bbox, torch.cat([predn[:, :4], predn[:, -1:]], dim=-1))
            if is_obb
            else box_iou(tgt_bbox, predn[:, :4])
        ) #torch.Size([17, 8])  这里的confusion_matrix_conf 0.25

        iou_posi = torch.where(bbox_iou > self.confusion_matrix_iou) #torch.Size([8])  iou过滤   0.45
        if iou_posi[0].shape[0]:
            iou_posi_value = torch.cat((torch.stack(iou_posi, 1), bbox_iou[iou_posi[0], iou_posi[1]][:, None]), 1).cpu().numpy() #
            if iou_posi[0].shape[0] > 1:
                iou_posi_value = iou_posi_value[iou_posi_value[:, 2].argsort()[::-1]]#置信度从大到小重排
                iou_posi_value = iou_posi_value[np.unique(iou_posi_value[:, 1], return_index=True)[1]] #一个真实框对应多个预测框 (8, 3)
                iou_posi_value = iou_posi_value[iou_posi_value[:, 2].argsort()[::-1]]
                iou_posi_value = iou_posi_value[np.unique(iou_posi_value[:, 0], return_index=True)[1]] # (7, 3)
        else:
            iou_posi_value = np.zeros((0, 3))

        have_match = iou_posi_value.shape[0] > 0  


        calculate_tgt_index, calculate_predn_index, _ = iou_posi_value.transpose().astype(int)

        # 遍历每个真实框的类别
        for i, tgt_cls in enumerate(tgt_cls_int):

            TF = calculate_tgt_index == i  
            if have_match and sum(TF) == 1:  # 如果存在唯一匹配
                # 将匹配的预测框类别与真实框类别在混淆矩阵中进行更新
                self.matrix[predn_cls_int[calculate_predn_index[TF]], tgt_cls] += 1  #混淆矩阵格式是先预测后真实
            else:
                # 如果没有匹配或有多个匹配，视为真实框的漏检
                self.matrix[self.nc, tgt_cls] += 1  
...
def ap_per_class(
    tp, conf, pd_cls, tgt_cls, plot=False, on_plot=None, save_dir=Path(), names=(), eps=1e-16, prefix=""
):

    # Sort by objectness
    i = np.argsort(-conf)
    tp, conf, pd_cls = tp[i], conf[i], pd_cls[i] #按照置信度从大到小排序
    # true positive  (23317,)代表23317个框
    # Find unique classes
    unique_tgt_cls, unique_tgt_cls_sum = np.unique(tgt_cls, return_counts=True) #929个目标 71个列别，每个类别的数目
    num_cls = unique_tgt_cls.shape[0]  # number of classes, number of predn

    # Create Precision-Recall curve and compute AP for each class
    x, map_50 = np.linspace(0, 1, 1000), []

    # Average precision, precision and recall curves (71, 10)类别 10档置信度   (71, 1000) 类别 1000
    ap, p_curve, r_curve = np.zeros((num_cls, tp.shape[1])), np.zeros((num_cls, 1000)), np.zeros((num_cls, 1000))
    for i, unique_tgt_cls_i in enumerate(unique_tgt_cls):
        cls_TF = pd_cls == unique_tgt_cls_i
        num_pred = cls_TF.sum()  # 这一类与多少预测数
        num_tgt = unique_tgt_cls_sum[i]  # number of labels  这一类有多少标签数
        ...
        # Recall
        recall = tpc / (num_tgt + eps)  # recall curve #(3737, 10)
        r_curve[i] = np.interp(-x, -conf[cls_TF], recall[:, 0], left=0)  # negative x, xp because xp decreases   iou阈值是0.5

        # Precision
        precision = tpc / (tpc + fpc)  # precision curve
        p_curve[i] = np.interp(-x, -conf[cls_TF], precision[:, 0], left=1)  # p at pr_score

        # AP from recall-precision curve
        for j in range(tp.shape[1]):
            ap[i, j], mpre, mrec = compute_ap(recall[:, j], precision[:, j]) #(71, 10) 71类 每一类10个档位 map是固定iou的结果
            if plot and j == 0:
                map_50.append(np.interp(x, mrec, mpre))  # precision at mAP@0.5

    map_50 = np.array(map_50)  # (num_cls, 1000)

    # Compute F1 (harmonic mean of precision and recall)
    f1_curve = 2 * p_curve * r_curve / (p_curve + r_curve + eps)
    names = [v for k, v in names.items() if k in unique_tgt_cls]  # list: only classes that have data
    names = dict(enumerate(names))  # to dict
    
    if plot:
        plot_pr_curve(x, map_50, ap, save_dir / f"{prefix}PR_curve.png", names, on_plot=on_plot)
    ...
    tp = (r * unique_tgt_cls_sum).round()  # true positives 找到的概率成总数目
    fp = (tp / (p + eps) - tp).round()  # false positives 总检测数-真确检测数
    return tp, fp, p, r, f1, ap, unique_tgt_cls.astype(int), p_curve, r_curve, f1_curve, x, map_50
...
class Metric(SimpleClass):
    ...
    def curves_results(self):
        """Returns a list of curves for accessing specific metrics curves."""
        return [
            [self.px, self.map_50, "Recall", "Precision"],
...
class DetMetrics(SimpleClass):
    ...
    def process(self, tp, conf, pd_cls, tgt_cls):
...
class SegmentMetrics(SimpleClass):
    ...
    Methods:
        process(tp_m, tp_b, conf, pd_cls, tgt_cls): Processes metrics over the given set of predictions.
    ...
    def process(self, tp, tp_m, conf, pd_cls, tgt_cls):
        """
        ...
        Args:
            ...
            tgt_cls (list): List of target classes.
        """
...
class PoseMetrics(SegmentMetrics):
    ...
    def process(self, tp, tp_p, conf, pd_cls, tgt_cls):
        """

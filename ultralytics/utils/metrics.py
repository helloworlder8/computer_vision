    ...
    if CIoU or DIoU or GIoU or EIoU or SIoU or WIoU:
        ...
        if CIoU or DIoU or EIoU or SIoU or WIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorconvex_hei/blob/master/utils/box/box_utils.py#L47
                ...
                angle_cost = torch.pow(1 - torch.exp(-(0.9847-angle_param)),3)  #5度
# # 计算 torch.arcsin(sin_best) 的值
# angle = torch.arcsin(torch.tensor(sin_best))

# # 计算最终表达式的值
# result = torch.cos(angle * 2 - math.pi / 2)
                """ 距离成本  原始  中心差比包围"""
                # min_w =  w2.minimum(w1)
                # min_h =  h2.minimum(h1)
                ...
                shape_cost = torch.pow(1 - torch.exp(-1 * omiga_w), 6) + torch.pow(1 - torch.exp(-1 * omiga_h), 6)
                if Focal:
...
class ConfusionMatrix:
    ...
    Attributes:
        task_name (str): The type of task_name, either 'detect' or 'classify'.
        matrix (np.ndarray): The confusion matrix, with dimensions depending on the task_name.
        nc (int): The number of classes.
        conf (float): The confidence threshold for detections.
        iou_thres (float): The Intersection over Union threshold.
    """

    def __init__(self, nc, conf=0.25, iou_thres=0.45, task_name="detect"):
        """Initialize attributes for the YOLO model."""
        self.task_name = task_name
        self.matrix = np.zeros((nc + 1, nc + 1)) if self.task_name == "detect" else np.zeros((nc, nc))
    ...
    def process_cls_preds(self, preds, targets):
        """
        Update confusion matrix for classification task_name.
    ...
    def process_batch(self, detections, gt_bboxes, gt_cls):
        """
        Update confusion matrix for object detection task_name.
    ...
    def tp_fp(self):
        ...
        return (tp[:-1], fp[:-1]) if self.task_name == "detect" else (tp, fp)  # remove background class if task_name=detect

    ...
    3. Use the command line interface (CLI):
        ...
        YOLOv8 'yolo' CLI commands use the following syntax:
            ...
            Where   TASK (optional) is one of [detect, segment, classify]
                    ...
                    ARGS (optional) are any number of custom 'old_arg=value' pairs like 'imgsz=320' that override defaults.
...
DEFAULT_PARAM_DICT = yaml_load(DEFAULT_CFG_PATH)
for k, v in DEFAULT_PARAM_DICT.items():
    if isinstance(v, str) and v.lower() == "none":
        DEFAULT_PARAM_DICT[k] = None
DEFAULT_CFG_KEYS = DEFAULT_PARAM_DICT.keys()
DEFAULT_PARAM = IterableSimpleNamespace(**DEFAULT_PARAM_DICT)
...
def deprecation_warn(old_arg, new_arg, version=None):
    """Issue a deprecation warning when a deprecated argument is used, suggesting an updated argument."""
    if not version:
        version = float(__version__[:3]) + 0.2  # deprecate after 2nd major release
    LOGGER.warning(
        f"WARNING ⚠️ '{old_arg}' is deprecated and will be removed in 'ultralytics {version}' in the future. "

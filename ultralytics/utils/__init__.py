...
def yaml_load(file="data.yaml", append_filename=False):
    ...
    with open(file, errors="ignore", encoding="utf-8") as f:
        ...
        data_dict = yaml.safe_load(s) or {}  # always return a dict (yaml.safe_load() may return None for empty files)
        if append_filename:
            data_dict["data_yaml"] = str(file)
        return data_dict


def yaml_print(model_yaml: Union[str, Path, dict]) -> None:
    """
    Pretty prints a YAML file or a YAML-formatted dictionary.

    Args:
        model_yaml: The file path of the YAML file or a YAML-formatted dictionary.

    Returns:
        (None)
    """
    yaml_dict = yaml_load(model_yaml) if isinstance(model_yaml, (str, Path)) else model_yaml
    dump = yaml.dump(yaml_dict, sort_keys=False, allow_unicode=True, width=float("inf"))
    LOGGER.info(f"Printing '{colorstr('bold', 'black', model_yaml)}'\n\n{dump}")
...
class SettingsManager(dict):
    ...
    def __init__(self, file=SETTINGS_YAML, version="0.0.4"):
        ...
        root = Path()
        datasets_root = root.parent if is_dir_writeable(root.parent) else root.resolve()
        ...
        self.defaults = {
            ...
            "weights_dir": str(root / "weights"), #默认权重文件路径

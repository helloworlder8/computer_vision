...
from ultralytics.cfg_yaml import TASK_TO_DATA_CFG, TASK2METRIC
...
def benchmark(
    ...
    Args:
        ...
        data (str, optional): Dataset to evaluate on, inherited from TASK_TO_DATA_CFG if not passed. Default is None.
    ...
    for i, (name, format, suffix, cpu, gpu) in export_formats().iterrows():  # index, (name, format, suffix, CPU, GPU)
        ...
        try:
            ...
            if i == 9:  # Edge TPU
                ...
                assert model.task_name != "obb", "TensorFlow GraphDef not supported for OBB task_name"
            ...
            # Export
            if format == "-":
                filename = model.model_str or model.cfg
                exported_model = model  # PyTorch format
            else:
                filename = model.export(imgsz=imgsz, format=format, half=half, int8=int8, device=device, verbose=False)
                exported_model = YOLO(filename, task_name=model.task_name)
                assert suffix in str(filename), "export failed"
            emoji = "❎"  # indicates export succeeded

            # Predict
            assert model.task_name != "pose" or i != 7, "GraphDef Pose inference is not supported"
            assert i not in (9, 10), "inference not supported"  # Edge TPU and TF.js are unsupported
            assert i != 5 or platform.system() == "Darwin", "inference only supported on macOS>=10.13"  # CoreML
            exported_model.predict(ASSETS / "bus.jpg", imgsz=imgsz, device=device, half=half)

            # Validate
            data = data or TASK_TO_DATA_CFG[model.task_name]  # task_name to dataset, i.e. coco8.yaml for task_name=detect
            key = TASK2METRIC[model.task_name]  # task_name to metric, i.e. metrics/mAP50-95(B) for task_name=detect
    ...
    name = Path(model.model_str).name

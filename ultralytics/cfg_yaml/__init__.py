...
def check_args(args_dict, hard=True):
    """Validate Ultralytics configuration argument types and values, converting them if necessary."""
    # Special handling for numeric project/name
    for k in "project", "name":
        if k in args_dict and isinstance(args_dict[k], (int, float)):
            args_dict[k] = str(args_dict[k]) #"project", "name"改成str
    if args_dict.get("name") == "model":  # assign model to 'name' arg
...
def get_save_dir(args, name=None):
    ...
    if getattr(args, "save_dir", None):
        save_dir = args.save_dir #参数中直接带有保存路径的情况
    else:
...
def check_dict_alignment(default_cfg_dict: Dict, overrides_dict: Dict, e=None):
    ...
    Example:
        ...
        custom_cfg = {'current_epoch': 100, 'lr': 0.02, 'batch_size': 32}

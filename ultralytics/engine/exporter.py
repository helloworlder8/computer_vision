...
class Exporter:
    ...
    def __init__(self, cfg=DEFAULT_PARAM, overrides=None, _callbacks=None):
        ...
        Args:
            cfg (str, optional): Path to a configuration file. Defaults to DEFAULT_PARAM.
            overrides (dict, optional): Configuration overrides. Defaults to None.
            _callbacks (dict, optional): Dictionary of callback functions. Defaults to None.
        """
        self.args = creat_args(cfg, overrides)
    ...
    def __call__(self, model=None):
        ...
        if any(f):
            ...
            predict_data = f"data={data}" if model.task_name == "segment" and fmt == "pb" else ""
            q = "int8" if self.args.int8 else "half" if self.args.half else ""  # quantization
            LOGGER.info(
                f'\nExport complete ({time.time() - t:.1f}s)'
                f"\nResults saved to {colorstr('bold', file.parent.resolve())}"
                f'\nPredict:         yolo predict task_name={model.task_name} model={f} imgsz={imgsz} {q} {predict_data}'
                f'\nValidate:        yolo val task_name={model.task_name} model={f} imgsz={imgsz} data={data} {q} {s}'
    ...
    def export_onnx(self, prefix=colorstr("ONNX:")):
        ...
        if dynamic:
            ...
            elif isinstance(self.model, Detection_Model):
    ...
    def export_openvino(self, prefix=colorstr("OpenVINO:")):
        ...
        def serialize(ov_model, file):
            ...
            if self.model.task_name != "classify":
        ...
        if self.args.int8:
            ...
            if not self.args.data:
                self.args.data = DEFAULT_PARAM.data or "coco128.yaml"
                LOGGER.warning(
    ...
    def export_coreml(self, prefix=colorstr("CoreML:")):
        ...
        if self.model.task_name == "classify":
            ...
            model = self.model
        elif self.model.task_name == "detect":
        ...
        if self.args.nms and self.model.task_name == "detect":
            if mlmodel:
    ...
    def _add_tflite_metadata(self, file):
        ...
        if self.model.task_name == "segment":
        ...
        subgraph.outputTensorMetadata = [output1, output2] if self.model.task_name == "segment" else [output1]

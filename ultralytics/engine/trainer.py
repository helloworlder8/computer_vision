...
class BaseTrainer:
    ...
    def get_dataloaders_validator(self, batch_size):
        ...
        if RANK in {-1, 0}:
            ...
            self.metrics_dict = dict(zip(metric_keys, [0] * len(metric_keys))) #指标字典
    ...
    #             self.metrics_dict, self.fitness = self.validate()
    #         self.save_metrics(metrics={**self.prefix_loss_items(self.avg_loss_items), **self.metrics_dict, **self.lr})
        ...
        torch.save(
            {
                ...
                "train_metrics": {**self.metrics_dict, **{"fitness": self.fitness}},
    ...
    def validate(self):
        ...
        """
        metrics_dict = self.validator(self)
        fitness = metrics_dict.pop("fitness", -self.loss.detach().cpu().numpy())  # use loss as fitness measure if not found
        if not self.best_fitness or self.best_fitness < fitness:
            self.best_fitness = fitness
        return metrics_dict, fitness
        ...
        for f in self.last, self.best:
            if f.exists():
                ...
                if f is self.best:
                    ...
                    self.metrics_dict = self.validator(model=f)
                    self.metrics_dict.pop("fitness", None)
                    self.run_callbacks("on_fit_epoch_end")

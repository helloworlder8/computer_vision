...
class BaseValidator:
    ...
    def create_tp_iouv_matrix(self, pd_cls, gt_cls, bbox_iou, use_scipy=False):
        """
        ...
        for i, threshold in enumerate(self.iouv.cpu().tolist()):
            if use_scipy:
                ...
                iou_index = np.nonzero(bbox_iou >= threshold)  # IoU > threshold and classes match 
                iou_index = np.array(iou_index).T #->(20, 2)
                if iou_index.shape[0]:
                    if iou_index.shape[0] > 1:
                        iou_index = iou_index[bbox_iou[iou_index[:, 0], iou_index[:, 1]].argsort()[::-1]] #(值排序 索引排序 -》(20, 2)
                    ...
                    tp_iouv_matrix[iou_index[:, 1].astype(int), i] = True #预测情况

...
class BasePredictor:
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        ...
        if self.args.conf is None:
            self.args.conf = 0.3  # default conf=0.25
    ...
    def stream_inference(self, source=None, model_name=None, *args, **kwargs):
        ...
        with self._lock:  # for thread-safe inference
            ...
            for self.batch in self.dataset:
                ...
                paths, ori_img, s = self.batch #图像路径 图像矩阵(1080, 810, 3) 当前图像索引original

                # Preprocess
                with profilers[0]:
                    process_img = self.preprocess(ori_img) #填充 归一化 数组变成张量
                ...
                with profilers[1]:
                    ...
                    preds = self.inference(process_img, *args, **kwargs)
                    if self.args.embed:
                ...
                # Postprocess
                with profilers[2]:
                    self.results = self.postprocess(preds, process_img, ori_img) #掩膜 得分 处理后的图像 原始图像
                ...
                for i in range(n):
                    ...
                    if self.args.verbose or self.args.save or self.args.save_txt or self.args.show:
                        s[i] += self.write_results(i, Path(paths[i]), process_img, s) #检测结果信息
        ...
        if self.args.verbose and self.seen:
            ...
            LOGGER.info(
                ...
                f"{(min(self.args.batch, self.seen), 3, *process_img.shape[2:])}" % t
            )

...
from ultralytics.nn.tasks import attribute_assignment, guess_model_task, nn, create_model_dict,load_download_model
...
class BaseProject(nn.Module):
    ...
    def _set_init_properties(self,task):
        self.callbacks = callbacks.get_default_callbacks() #class defaultdict
    ...
    def load(self, weights: Union[str, Path] = "yolov8n.pt") -> "BaseProject":
        ...
        if isinstance(weights, (str, Path)):
            self.ckpt, weights = attribute_assignment(weights)
    ...
    def info(self, detailed: bool = False, verbose: bool = True):
        ...
        """
        self._check_is_pytorch_model()
        return self.model.model_info(detailed=detailed, verbose=verbose)
    ...
    def _combine_args(self, kwargs):
        #  task data imgsz single_cls model_name
        ...
        self.args = {**self.overrides, **self.custom, **kwargs, "mode": "train"} #多了轮次和模式
    ...
    def _initialize_trainer(self, kwargs):
        ...
        if not self.args.get("resume"):
            ...
            self.trainer.model = self.trainer.get_model(cfg=self.model.model_dict, weights=self.model if self.ckpt else None )  #新建或者加载生成的模型
    ...
    def _update_model_and_cfg(self):
        ...
        if RANK in {-1, 0}:
            ...
            _, self.model = attribute_assignment(ckpt)

    ...
    def plot(
        ...
        Example:
            ...
            for r in results:
                ...
                img.show()  # show image
                img.save('results.jpg')  # save image
            ```
        """
        ...
        pd_seg_matrix, show_masks = self.masks, masks
        ...
        if pd_seg_matrix and show_masks:
            if im_gpu is None:
                img = LetterBox(pd_seg_matrix.shape[1:])(image=annotator.result())
                im_gpu = (
                    torch.as_tensor(img, dtype=torch.float16, device=pd_seg_matrix.data.device)
                    .permute(2, 0, 1)
                    .flip(0)
                    .contiguous()
                    / 255
                )
            idx = pred_boxes.cls if pred_boxes else range(len(pd_seg_matrix))
            annotator.masks(pd_seg_matrix.data, colors=[colors(x, True) for x in idx], im_gpu=im_gpu)

...
class Results(SimpleClass):
    ...
    def plot(
        ...
        pd_masks, show_masks = self.masks, masks
        ...
        if pd_masks and show_masks:
            if im_gpu is None:
                img = LetterBox(pd_masks.shape[1:])(image=annotator.result())
                im_gpu = (
                    torch.as_tensor(img, dtype=torch.float16, device=pd_masks.data.device)
                    .permute(2, 0, 1)
                    .flip(0)
                    .contiguous()
                    / 255

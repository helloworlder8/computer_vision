...
class BaseDataset(Dataset):
    ...
    def __init__(
        ...
        self.max_buffer_length = min((self.ni, self.bs * 8, 1000)) if self.augment else 0
    ...
    def load_image(self, i, rect_mode=True):
        """Loads 1 image from dataset index 'i', returns (img, resized hw)."""
        img, f, fn = self.ims[i], self.im_files[i], self.npy_files[i]
        if img is None:  # not cached in RAM
            ...
            if self.augment:
                self.ims[i], self.im_hw0[i], self.im_hw[i] = img, (h0, w0), img.shape[:2]  # img, hw_original, hw_resized
        ...
        with ThreadPool(NUM_THREADS) as pool:
            ...
            for i, x in pbar:
                if self.cache == "disk":
                    ...
                    self.ims[i], self.im_hw0[i], self.im_hw[i] = x  # img, hw_orig, hw_resized = load_image(self, i)
                    b += self.ims[i].nbytes
    ...
    def check_cache_ram(self, safety_margin=0.5):
        ...
        for _ in range(n):
            img = cv2.imread(random.choice(self.im_files))  # sample image
            ratio = self.imgsz / max(img.shape[0], img.shape[1])  # max(h, w)  # ratio
    ...
    def set_rectangle(self):
        ...
        bi = np.floor(np.arange(self.ni) / self.bs).astype(int)  # batch index

...
def check_det_dataset(data_yaml, autodownload=True): #yaml数据字符串路径
    """
    ...
    data_yaml = check_file(data_yaml) #单纯检测文件是否存在

    # Download (optional)
    extract_dir = ""
    if zipfile.is_zipfile(data_yaml) or is_tarfile(data_yaml):
        new_dir = safe_download(data_yaml, dir=DATASETS_DIR, unzip=True, delete=False)
        data_yaml = find_dataset_yaml(DATASETS_DIR / new_dir)
        extract_dir, autodownload = data_yaml.parent, False

    # Read YAML
    data_dict = yaml_load(data_yaml, append_filename=True)  # dictionary

    # Checks
    for k in "train", "val":
        if k not in data_dict:
            if k != "val" or "validation" not in data_dict:
                raise SyntaxError(
                    emojis(f"{data_yaml} '{k}:' key missing ❌.\n'train' and 'val' are required in all data YAMLs.")
                )
    ...
    if "names" in data_dict and "nc" in data_dict and len(data_dict["names"]) != data_dict["nc"]: #都有但是两个数量不相等
        raise SyntaxError(emojis(f"{data_yaml} 'names' length {len(data_dict['names'])} and 'nc: {data_dict['nc']}' must match."))
    if "names" not in data_dict: #没名字 我给你取名字
    ...
    # Resolve paths
    path = Path(extract_dir or data_dict.get("path") or Path(data_dict.get("model_yaml", "")).parent)  # dataset root
    if not path.is_absolute():
        path = (DATASETS_DIR / path).resolve() #这里是数据集路径

    # Set paths
    data_dict["path"] = path  # download scripts
    for k in "train", "val", "test", "minival":
        if data_dict.get(k):  # prepend path
            if isinstance(data_dict[k], str):
                x = (path / data_dict[k]).resolve()
    ...
    if val:
        ...
        if not all(x.exists() for x in val):
            name = clean_url(data_yaml)  # dataset name with URL auth stripped
...
def compress_one_image(f, f_new=None, max_dim=1920, quality=50):
    ...
    try:  # use PIL
        img = Image.open(f)
        r = max_dim / max(img.height, img.width)  # ratio
        if r < 1.0:  # image too large
            img = img.resize((int(img.width * r), int(img.height * r)))
        img.save(f_new or f, "JPEG", quality=quality, optimize=True)  # save
        ...
        if r < 1.0:  # image too large
            img = cv2.resize(img, (int(im_width * r), int(im_height * r)), interpolation=cv2.INTER_AREA)
        cv2.imwrite(str(f_new or f), img)

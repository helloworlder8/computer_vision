...
class CopyPaste:
    ...
    def __call__(self, labels):
        ...
        if self.p and len(instances.segments):
            n = len(instances)
            _, w, _ = img.shape  # height, width, channels
    ...
    def __call__(self, labels):
        ...
        if self.contains_spatial:
            ...
            if len(cls):
                ...
                new = self.transform(image=img, bboxes=bboxes, class_labels=cls)  # transformed
                if len(new["class_labels"]) > 0:  # skip update if no bbox in new img
...
class RandomLoadText:
    ...
    def __call__(self, labels: dict) -> dict:
        ...
        neg_samples = min(min(nc, self.max_samples) - len(pos_labels), random.randint(*self.neg_samples))
        neg_labels = [i for i in range(nc) if i not in pos_labels]
...
def v8_transforms(dataset, imgsz, hyp, stretch=False):
    ...
    flip_idx = dataset.data_dict.get("flip_idx", [])  # for keypoints augmentation
...
class ClassifyLetterBox:
    ...
    def __call__(self, img):
        ...
        Args:
            img (numpy.ndarray): The input image as a numpy array of shape HWC.
        ...
        # Create padded image
        im_out = np.full((hs, ws, 3), 114, dtype=img.dtype)
...
class CenterCrop:
    ...
    def __call__(self, img):
        ...
        """
        if isinstance(img, Image.Image):  # convert from PIL to numpy array if required
            img = np.asarray(img)
        imh, imw = img.shape[:2]
        m = min(imh, imw)  # min dimension
        top, left = (imh - m) // 2, (imw - m) // 2
        return cv2.resize(img[top : top + m, left : left + m], (self.w, self.h), interpolation=cv2.INTER_LINEAR)
...
class ToTensor:
    ...
    def __call__(self, img):
        ...
        Args:
            img (numpy.ndarray): Input image as a numpy array with shape (H, W, C) in BGR order.

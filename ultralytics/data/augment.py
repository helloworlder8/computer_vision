...
from .verify import polygons2masks, polygons2masks_overlap
...
class Compose:
    ...
    def __init__(self, transforms):
        ...
        self.transforms = transforms#三个类结合在一起
...
class BaseMixTransform:
    ...
    def __init__(self, dataset_class, pre_transform=None, p=0.0) -> None:
        """Initializes the BaseMixTransform object with dataset_class, pre_transform, and probability."""
    ...
    def __call__(self, labels):
        ...
        mix_labels = [self.dataset_class.get_image_and_label(i) for i in indexes]
...
class Mosaic(BaseMixTransform):
    ...
    The augmentation is applied to a dataset_class with a given probability.

    Attributes:
        dataset_class: The dataset_class on which the mosaic augmentation is applied.
        imgsz (int, optional): Image size (height and width) after mosaic pipeline of a single image. Default to 640.
        p (float, optional): Probability of applying the mosaic augmentation. Must be in the range 0-1. Default to 1.0.
        n (int, optional): The grid size, either 4 (for 2x2) or 9 (for 3x3).
    """

    def __init__(self, dataset_class, imgsz=640, p=1.0, n=4):
        """Initializes the object with a dataset_class, image size, probability, and border."""
        assert 0 <= p <= 1.0, f"The probability should be in range [0, 1], but got {p}."
        assert n in (4, 9), "grid must be equal to 4 or 9."
        super().__init__(dataset_class=dataset_class, p=p)
        self.dataset_class = dataset_class
        self.imgsz = imgsz
    ...
    def get_indexes(self, buffer=True):
        """Return a list of random indexes from the dataset_class."""
        if buffer:  # select images from buffer
            return random.choices(list(self.dataset_class.buffer), k=self.n - 1)
        else:  # select any images
            return [random.randint(0, len(self.dataset_class) - 1) for _ in range(self.n - 1)]
...
class MixUp(BaseMixTransform):
    """Class for applying MixUp augmentation to the dataset_class."""

    def __init__(self, dataset_class, pre_transform=None, p=0.0) -> None:
        """Initializes MixUp object with dataset_class, pre_transform, and probability of applying MixUp."""
        super().__init__(dataset_class=dataset_class, pre_transform=pre_transform, p=p)

    def get_indexes(self):
        """Get a random index from the dataset_class."""
        return random.randint(0, len(self.dataset_class) - 1)
...
class RandomPerspective:
    """
    Implements random perspective透视 and affine仿射 transformations on images and corresponding bounding boxes, segments, and
    ...
    Attributes:
        degrees (float): Degree range for random rotations.随机旋转的角度
        translate (float): Fraction of total width and height for random translation.衡量随机平移宽高
        scale (float): Scaling factor interval, e.g., a scale factor of 0.1 allows a resize between 90%-110%.缩放因子
        shear (float): Shear intensity (angle in degrees).剪切角度
        perspective (float): Perspective distortion factor.透视失真系数
        border (tuple): Tuple specifying mosaic border.边界框
    ...
    def __init__(
        ...
        # 中心平移(Center)、透视变换(Perspective)、旋转(Rotation)、缩放(Scale)、错切(Shear)、Translation(平移)封装在一起
    ...
    def affine_transform(self, img, border):
        ...
        # Center
        C = np.eye(3, dtype=np.float32) #单位矩阵
...
def v8_transforms(dataset_class, imgsz, hyp, stretch=False):
    """Convert images to a size suitable for YOLOv8 training."""
    pre_transform = Compose(
        [
            Mosaic(dataset_class, imgsz=imgsz, p=hyp.mosaic),
    ...
    flip_idx = dataset_class.data_dict.get("flip_idx", [])  # for keypoints augmentation
    if dataset_class.use_keypoints:
        kpt_shape = dataset_class.data.get("kpt_shape", None)
    ...
    return Compose(
        [
            pre_transform,
            MixUp(dataset_class, pre_transform=pre_transform, p=hyp.mixup),

...
class ClassificationTrainer(Engine_Trainer):
    """
    A class extending the Engine_Trainer class for training based on a classification model.
    ...
    def build_model(self, model_str=None, model=None, verbose=True):
        ...
        classification_Model = ClassificationModel(model_dict, ch=self.data_dict["ch"], nc=self.data_dict["nc"], verbose=verbose and RANK == -1)
        if model:
            classification_Model.load(model)
    ...
    def _setup_model(self):
        ...
        if model.endswith(".pt"):
            self.model, ckpt = load_pytorch_model(model, device="cpu")
    ...
    def get_dataloader(self, dataset_path, batch_size=16, rank=0, mode="train"):
        ...
        loader = creat_dataloader(dataset, batch_size, self.args.workers, rank=rank)

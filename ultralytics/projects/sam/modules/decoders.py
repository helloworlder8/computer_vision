...
class MaskDecoder(nn.Module):
    ...
    def forward(
        ...
        points_boxes_embeddings: torch.Tensor,
        ...
        Args:
            ...
            points_boxes_embeddings (torch.Tensor): the embeddings of the points and boxes
        ...
        masks, iou_pred = self.predict_masks(
            ...
            points_boxes_embeddings=points_boxes_embeddings,
    ...
    def predict_masks(
        ...
        iou_mask_token = torch.cat([self.iou_token.weight, self.mask_tokens.weight], dim=0)
        iou_mask_token = iou_mask_token.unsqueeze(0).expand(points_boxes_embeddings.shape[0], -1, -1)
        tokens = torch.cat((iou_mask_token, points_boxes_embeddings), dim=1)

        # Expand per-image data in batch direction to be per-mask
        src = torch.repeat_interleave(image_embeddings, tokens.shape[0], dim=0)
        src = src + dense_prompt_embeddings
        pos_src = torch.repeat_interleave(positional_embeddings, tokens.shape[0], dim=0)
        b, c, h, w = src.shape

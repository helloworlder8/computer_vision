    ...
    def prompt(self, results, bboxes=None, points=None, labels=None, texts=None):
        ...
        for result in results: #遍历每一张图像
            ...
            if bboxes is not None:
                ...
                bbox_areas = (bboxes[:, 3] - bboxes[:, 1]) * (bboxes[:, 2] - bboxes[:, 0]) #每个框的面积
                regions_masks = []
                for b in bboxes:  #遍历每一个提示框
                    regions_masks.append(masks[:, b[1] : b[3], b[0] : b[2]].sum(dim=(1, 2))) #框内掩膜像素数目
                mask_areas = torch.stack(regions_masks)

                full_mask_areas = torch.sum(masks, dim=(1, 2))

                union = bbox_areas[:, None] + full_mask_areas - mask_areas
                idx[torch.argmax(mask_areas / union, dim=1)] = True #选出做好的掩码 就是框就一个

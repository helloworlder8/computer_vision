...
class PoseValidator(DetectionValidator):
    ...
    def init_metrics(self, model):
        ...
        self.stats_dict = dict(tp_p=[], tp=[], conf=[], pd_cls=[], gt_cls=[], gt_unique_cls=[])

    def _get_gt_dict_per_image(self, si, batch):
        """Prepares a batch for processing by converting keypoints to float and moving to device."""
        pbatch = super()._get_gt_dict_per_image(si, batch)
    ...
    def _generate_native_space_predn(self, pred, pbatch):
    ...
    def update_metrics_per_batch(self, preds, batch):
        ...
        for si, pred in enumerate(preds):
            ...
            if self.args.single_cls:
                pred[:, 5] = 0
            predn, pred_kpts = self._generate_native_space_predn(pred, pbatch)
            stat["conf"] = predn[:, 4]
            stat["pd_cls"] = predn[:, 5]
            ...
            if nl:
                ...
                stat["tp_p"] = self._generate_per_image_tp(predn, bbox, cls, pred_kpts, pbatch["kpts"])
                if self.args.plots:

...
class SegmentationValidator(DetectionValidator):
    ...
    def init_metrics(self, model):
        ...
        self.stats_dict = dict(tp_m=[], tp=[], conf=[], pd_cls=[], gt_cls=[], gt_unique_cls=[])
    ...
    def _get_gt_dict_per_image(self, si, batch):
        """Prepares a batch for training or inference by processing images and targets."""
        prepared_batch = super()._get_gt_dict_per_image(si, batch)
        midx = [si] if self.args.overlap_mask else batch["img_idx"] == si
        prepared_batch["masks"] = batch["masks"][midx]
        return prepared_batch

    def _generate_native_space_predn(self, pred, pbatch, proto):
        """Prepares a batch for training or inference by processing images and targets."""
        predn = super()._generate_native_space_predn(pred, pbatch)
    ...
    def update_metrics_per_batch(self, preds, batch):
        ...
        for si, (pred, proto) in enumerate(zip(preds[0], preds[1])):
            ...
            pbatch = self._get_gt_dict_per_image(si, batch)
            cls, bbox = pbatch.pop("cls"), pbatch.pop("bbox")
            nl = len(cls)
            stat["gt_cls"] = cls
            stat["gt_unique_cls"] = cls.unique()
            if npr == 0:
                if nl:
            ...
            if self.args.single_cls:
                pred[:, 5] = 0
            predn, pd_masks = self._generate_native_space_predn(pred, pbatch, proto)
            stat["conf"] = predn[:, 4]
            stat["pd_cls"] = predn[:, 5]
    ...
    def _generate_per_image_tp(self, predn, gt_bboxes, gt_cls, pd_masks=None, gt_masks=None, overlap=False, masks=False):
        """
        ...
        Example:
            ...
            correct_preds = validator._generate_per_image_tp(predn, gt_bboxes, gt_cls)
            ```
        """
        if masks:
            if overlap:

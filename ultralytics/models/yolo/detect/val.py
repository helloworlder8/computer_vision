...
class DetectionValidator(BaseValidator):
    ...
    def _generate_native_space_predn(self, pred, gt_dict):
        """Prepares a batch of images and annotations for validation."""
        predn = pred.clone() #深拷贝 不要改变pred
        ops.scale_boxes(
            gt_dict["imgsz"], predn[:, :4], gt_dict["ori_shape"], ratio_pad=gt_dict["ratio_pad"]
        )  # native-space pred
        return predn
    ...
    def update_metrics_per_batch(self, preds, batch):
        ...
        for img_index, pred in enumerate(preds): #对每一张图片的操作
            ...
            gt_dict = self._get_gt_dict_per_image(img_index, batch) #17个目标
            gt_cls, gt_bbox = gt_dict.pop("cls"), gt_dict.pop("bbox")
            num_gt = len(gt_cls) #-》17
            stat["gt_cls"] = gt_cls #真是的有多少个框
            stat["gt_unique_cls"] = gt_cls.unique() #一张图片一个类最多一个框
            if num_pred == 0:
                if num_gt:
    ...
    def generate_metrics_info(self):
        ...
        self.nt_per_image = np.bincount(stats["gt_unique_cls"].astype(int), minlength=self.nc) #(80,) 这玩意也没用到啊
        stats.pop("gt_unique_cls", None)

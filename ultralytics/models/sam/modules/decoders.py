...
class MaskDecoder(nn.Module):
    ...
    def predict_masks(
        ...
        repeat_image_embeddings = torch.repeat_interleave(image_embeddings, tokens.shape[0], dim=0)
        repeat_image_embeddings = repeat_image_embeddings + dense_prompt_embeddings
        pos_src = torch.repeat_interleave(image_pe, tokens.shape[0], dim=0)
        b, c, h, w = repeat_image_embeddings.shape

        # Run the transformer
        tokens_out, repeat_image_embeddings = self.transformer(repeat_image_embeddings, pos_src, tokens)
        iou_token_out = tokens_out[:, 0, :]
        mask_tokens_out = tokens_out[:, 1 : (1 + self.num_mask_tokens), :]

        # Upscale mask embeddings and predict masks using the mask tokens
        repeat_image_embeddings = repeat_image_embeddings.transpose(1, 2).view(b, c, h, w)
        upscaled_embedding = self.output_upscaling(repeat_image_embeddings)
        mask_tokens_list: List[torch.Tensor] = [
            self.output_hypernetworks_mlps[i](mask_tokens_out[:, i, :]) for i in range(self.num_mask_tokens)
        ]
        mask_tokens_stack = torch.stack(mask_tokens_list, dim=1)
        b, c, h, w = upscaled_embedding.shape
        masks = (mask_tokens_stack @ upscaled_embedding.view(b, c, h * w)).view(b, -1, h, w)
...
class SAM2MaskDecoder(nn.Module):
    ...
    def forward(
        ...
        if multimask_output:
            ...
            masks, iou_pred = self._dynamic_multimask_via_stability(masks, iou_pred) #torch.Size([3, 4, 256, 256]) torch.Size([3, 4])->torch.Size([3, 1, 256, 256]) torch.Size([3, 1])
        else:
            masks = masks[:, 0:1, :, :]
        ...
        if multimask_output and self.use_multimask_token_for_obj_ptr:
            ...
            sam_tokens_out = mask_tokens_out[:, 0:1]  # [b, 1, c] shape  torch.Size([3, 1, 256])

        # Prepare output torch.Size([3, 1, 256, 256]) torch.Size([3, 1]) torch.Size([3, 1, 256]) torch.Size([3, 1])
        return masks, iou_pred, sam_tokens_out, object_score_logits

    def predict_masks(
        self,
        image_embeddings: torch.Tensor, #torch.Size([1, 256, 64, 64])
        image_pe: torch.Tensor, #torch.Size([1, 256, 64, 64])
        sparse_prompt_embeddings: torch.Tensor, #torch.Size([3, 3, 256])
        dense_prompt_embeddings: torch.Tensor, #torch.Size([3, 256, 64, 64])
        repeat_image: bool, #true
        high_res_features: Optional[List[torch.Tensor]] = None, #[torch.Size([1, 32, 256, 256]), torch.Size([1, 64, 128, 128])]
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.pred_obj_scores:
            ...
            output_tokens = torch.cat([self.iou_token.weight, self.mask_tokens.weight], dim=0)
        output_tokens = output_tokens.unsqueeze(0).expand(sparse_prompt_embeddings.size(0), -1, -1) #torch.Size([3, 6, 256]) 提示 批 token数目 维度
        tokens = torch.cat((output_tokens, sparse_prompt_embeddings), dim=1) #torch.Size([3, 9, 256])
        # 每一组前6行是一样的
        # Expand per-image data in batch direction to be per-mask
        if repeat_image:
            repeat_image_embeddings = torch.repeat_interleave(image_embeddings, tokens.shape[0], dim=0)
            ...
            repeat_image_embeddings = image_embeddings
        repeat_image_embeddings = repeat_image_embeddings + dense_prompt_embeddings
...
# image_embedding
        # Run the transformer
        tokens_out, repeat_image_embeddings = self.transformer(repeat_image_embeddings, pos_src, tokens) #torch.Size([3, 256, 64, 64]) torch.Size([3, 256, 64, 64]) torch.Size([3, 9, 256])
        iou_token_out = tokens_out[:, s, :]
        mask_tokens_out = tokens_out[:, s + 1 : (s + 1 + self.num_mask_tokens), :]

        # Upscale mask embeddings and predict masks using the mask tokens
        repeat_image_embeddings = repeat_image_embeddings.transpose(1, 2).view(b, c, h, w) #批 维 点 点
        if not self.use_high_res_features:
            upscaled_embedding = self.output_upscaling(repeat_image_embeddings)
        else:
            dc1, ln1, act1, dc2, act2 = self.output_upscaling
            feat_s0, feat_s1 = high_res_features #[torch.Size([1, 32, 256, 256]), torch.Size([1, 64, 128, 128])]
            upscaled_embedding = act1(ln1(dc1(repeat_image_embeddings) + feat_s1)) #torch.Size([3, 64, 128, 128])
            upscaled_embedding = act2(dc2(upscaled_embedding) + feat_s0) #torch.Size([3, 32, 256, 256])

        mask_tokens_list: List[torch.Tensor] = []
        for i in range(self.num_mask_tokens):
            mask_tokens_list.append(self.output_hypernetworks_mlps[i](mask_tokens_out[:, i, :]))#torch.Size([3, 32])
        mask_tokens_stack = torch.stack(mask_tokens_list, dim=1) # torch.Size([3, 4, 32])
        b, c, h, w = upscaled_embedding.shape #3 32 256 256
        masks = (mask_tokens_stack @ upscaled_embedding.view(b, c, h * w)).view(b, -1, h, w) #torch.Size([1, 4, 256, 256])

        # Generate mask quality predictions
        iou_pred = self.iou_prediction_head(iou_token_out) #torch.Size([3, 256]) torch.Size([3, 4])
        if self.pred_obj_scores:
            assert s == 1
            object_score_logits = self.pred_obj_score_head(tokens_out[:, 0, :]) #torch.Size([1, 1])
        else:
            # Obj scores logits - default to 10.0, i.e. assuming the object is present, sigmoid(10)=1
            object_score_logits = 10.0 * iou_pred.new_ones(iou_pred.shape[0], 1)
        # torch.Size([1, 4, 256, 256]) #torch.Size([1, 4]) torch.Size([1, 4, 256]) #torch.Size([1, 1])
        return masks, iou_pred, mask_tokens_out, object_score_logits

    def _get_stability_scores(self, mask_logits):
        """Computes mask stability scores based on IoU between upper and lower thresholds."""
        mask_logits = mask_logits.flatten(-2) #torch.Size([1, 1, 65536])
        stability_delta = self.dynamic_multimask_stability_delta #0.05
    ...
    def _dynamic_multimask_via_stability(self, all_mask_logits, all_iou_scores):
        ...
        best_scores_inds = torch.argmax(multimask_iou_scores, dim=-1) #剩余的最佳得分
        batch_inds = torch.arange(multimask_iou_scores.size(0), device=all_iou_scores.device)
        best_multimask_logits = multimask_logits[batch_inds, best_scores_inds] #当前图像最好的掩膜
        best_multimask_logits = best_multimask_logits.unsqueeze(1) #torch.Size([1, 1, 256, 256])
        best_multimask_iou_scores = multimask_iou_scores[batch_inds, best_scores_inds]
        best_multimask_iou_scores = best_multimask_iou_scores.unsqueeze(1) #torch.Size([1, 1]) 当前图像最好的iou

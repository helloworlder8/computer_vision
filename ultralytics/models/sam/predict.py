...
class Predictor(BasePredictor):
    ...
    def preprocess(self, img):
        ...
        Args:
            img (torch.Tensor | List[np.ndarray]): BCHW tensor format or list of HWC numpy arrays.
        ...
        if not_tensor:
            img = np.stack(self.pre_transform(img)) #->(1, 1024, 1024, 3)
    ...
    def pre_transform(self, img):
        ...
        Args:
            img (List[np.ndarray]): List containing images in HWC numpy array format.
    ...
    def inference(self, img, bboxes=None, points=None, labels=None, masks=None, multimask_output=False, *args, **kwargs):
        ...
        Args:
            img (torch.Tensor): The preprocessed input image in tensor format, with shape (N, C, H, W).
    ...
    def prompt_inference(self, img, bboxes=None, points=None, labels=None, masks=None, multimask_output=False):
        ...
        Returns:
            (tuple): Contains the following three elements.
                ...
                - np.ndarray: Low-resolution logits of shape CxHxW for subsequent inference, where H=W=256.
        """
        features = self.model.image_encoder(img) if self.features is None else self.features #->torch.Size([1, 256, 64, 64])

        src_shape, dst_shape = self.batch[1][0].shape[:2], img.shape[2:]
        ...
        return pd_masks.flatten(0, 1), pd_scores.flatten(0, 1)
    ...
    def generate(
        ...
        Args:
            img (torch.Tensor): Input tensor representing the preprocessed image with dimensions (N, C, H, W).
        ...
        pd_masks, pd_scores, pd_bboxes, region_areas = [], [], [], []
        for crop_region, layer_idx in zip(crop_regions, layer_idxs):
            x1, y1, x2, y2 = crop_region
            w, h = x2 - x1, y2 - y1
            area = torch.tensor(w * h, device=img.device)
            points_scale = np.array([[w, h]])  # w, h
            # Crop image and interpolate to input size
            crop_im = F.interpolate(img[..., y1:y2, x1:x2], (ih, iw), mode="bilinear", align_corners=False)
            # (num_points, 2)
            ...
            keep = torchvision.ops.nms(crop_bboxes, crop_scores, self.args.NMS_Threshold)  # NMS
            crop_bboxes = uncrop_boxes_xyxy(crop_bboxes[keep], crop_region)
            crop_masks = uncrop_masks(crop_masks[keep], crop_region, ih, iw)
            crop_scores = crop_scores[keep]

            pd_masks.append(crop_masks)
            pd_bboxes.append(crop_bboxes)
            pd_scores.append(crop_scores)
            region_areas.append(area.expand(len(crop_masks)))

        pd_masks = torch.cat(pd_masks)
        pd_bboxes = torch.cat(pd_bboxes)
        pd_scores = torch.cat(pd_scores)
        region_areas = torch.cat(region_areas)

        # Remove duplicate masks between crops
        if len(crop_regions) > 1:
            scores = 1 / region_areas
            keep = torchvision.ops.nms(pd_bboxes, scores, crop_nms_thresh)
            pd_masks, pd_bboxes, pd_scores = pd_masks[keep], pd_bboxes[keep], pd_scores[keep]

        return pd_masks, pd_scores, pd_bboxes
    ...
    def postprocess(self, preds, img, orig_imgs):
        ...
        results = []
        for i, masks in enumerate([pd_masks]):
            orig_img = orig_imgs[i]
            if pd_bboxes is not None:
                pd_bboxes = ops.scale_boxes(img.shape[2:], pd_bboxes.float(), orig_img.shape, padding=False)
    ...
    def set_image(self, image):
        ...
        for batch in self.dataset:
            ...
            self.features = self.model.image_encoder(img)
            self.img = img
            break

...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, pd_bboxes, pd_scores, gt_bboxes, gt_cls, gt_groups, masks=None, gt_mask=None):
        ...
        pd_scores = pd_scores.detach().view(-1, nc)
        pd_scores = F.sigmoid(pd_scores) if self.use_fl else F.softmax(pd_scores, dim=-1)
        # [batch_size * num_queries, 4]
        pd_bboxes = pd_bboxes.detach().view(-1, 4)

        # Compute the classification cost
        pd_scores = pd_scores[:, gt_cls]
        if self.use_fl:
        ...
        cost_bbox = (pd_bboxes.unsqueeze(1) - gt_bboxes.unsqueeze(0)).abs().sum(-1)  # (bs*num_queries, num_gt)

        # Compute the GIoU cost between boxes, (bs*num_queries, num_gt)
        cost_giou = 1.0 - bbox_iou(pd_bboxes.unsqueeze(1), gt_bboxes.unsqueeze(0), xywh=True, GIoU=True).squeeze(-1)
...
def get_cdn_group(
    targets, nc, num_queries, class_embed, num_dn=100, cls_noise_ratio=0.5, box_noise_scale=1.0, training=False
):
    """
    ...
    Args:
        targets (dict): A dict that includes 'gt_cls' (torch.Tensor with shape [num_gts, ]), 'gt_bboxes'
    ...
    # Pad gt to max_num of a targets
    bs = len(gt_groups)
    ...
    if cls_noise_ratio > 0:
        ...
        new_label = torch.randint_like(idx, 0, nc, dtype=dn_cls.dtype, device=dn_cls.device)

...
#     # 还没加噪声的初始情况
#     dn_cls = gt_cls.repeat(2 * num_dn_group) #torch.Size([308])
#     dn_bbox = gt_bbox.repeat(2 * num_dn_group, 1) #torch.Size([308, 4])
#     dn_img_idx = gt_img_idx.repeat(2 * num_dn_group).view(-1) #torch.Size([308])
...
#     dn_cls_embed = dn_cls_embed[dn_cls] #torch.Size([308, 256]) 加入噪声的类编码
...
#     # torch.Size([2, 198, 4])
#     dn_pos = torch.cat([torch.tensor(range(num), dtype=torch.long) for num in bboxs_each_img]) #torch.Size([14])
#     dn_pos_idx = torch.stack([dn_pos + batch_max_box * i for i in range(num_dn_group)], dim=0) #torch.Size([14, 11])

#     dn_pos = torch.cat([dn_pos + batch_max_box * i for i in range(2 * num_dn_group)]) #torch.Size([308])
...
def generate_cdn_train_sample(gt, nc, nq, dn_embed, num_dn=100, cls_noise_ratio=0.5, box_noise_scale=1.0, training=False):
    ...
    # 噪声类编码与边界框编码
    dn_embed = dn_embed[dn_cls]  # torch.Size([308, 256]) 加入噪声的类编码

...
class DETRLoss(nn.Module):
    ...
    def _get_loss(
        ...
        if match_indices is None:
            match_indices = self.matcher(
                pd_bboxes, pd_scores, gt_bboxes, gt_cls, gt_groups, masks=masks, gt_mask=gt_mask
            )

        idx, gt_idx = self._get_index(match_indices)
        pd_bboxes, gt_bboxes = pd_bboxes[idx], gt_bboxes[gt_idx]

        bs, nq = pd_scores.shape[:2]
        targets = torch.full((bs, nq), self.nc, device=pd_scores.device, dtype=gt_cls.dtype)
        targets[idx] = gt_cls[gt_idx]

        gt_scores = torch.zeros([bs, nq], device=pd_scores.device)
        if len(gt_bboxes):
    ...
    def forward(self, pd_bboxes, pd_scores, batch, postfix="", **kwargs):
        """
        Args:
        ...
        total_loss = self._get_loss(
            pd_bboxes[-1], pd_scores[-1], gt_bboxes, gt_cls, gt_groups, postfix=postfix, match_indices=match_indices
        )

        if self.aux_loss:
            total_loss.update(
                self._get_loss_aux(
                    pd_bboxes[:-1], pd_scores[:-1], gt_bboxes, gt_cls, gt_groups, match_indices, postfix
                )
            )
...
class RTDETRDetectionLoss(DETRLoss):
    ...
    def forward(self, preds, batch, dn_bboxes=None, dn_scores=None, dn_meta=None):
        ...
        """
        pd_bboxes, pd_scores = preds
        total_loss = super().forward(pd_bboxes, pd_scores, batch)

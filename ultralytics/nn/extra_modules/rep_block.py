...
import numpy as np
from ultralytics.nn.modules.base_conv import Conv, autopad

# 当你使用from some_module import *语法时，Python会导入在some_module.__all__列表中定义的所有名称
...
class BNAndPadLayer(nn.Module):
    ...
    def forward(self, input): #torch.Size([16, 256, 80, 80])
        output = self.bn(input)  #torch.Size([16, 256, 80, 80])
        if self.pad_pixels > 0:
            if self.bn.affine:
                pad_values = self.bn.bias.detach() - self.bn.running_mean * self.bn.weight.detach() / torch.sqrt(self.bn.running_var + self.bn.eps)
            else:
                pad_values = - self.bn.running_mean / torch.sqrt(self.bn.running_var + self.bn.eps)
            output = F.pad(output, [self.pad_pixels] * 4) #torch.Size([16, 256, 82, 82])
            pad_values = pad_values.view(1, -1, 1, 1) #torch.Size([1, 256, 1, 1])
...
class DiverseBranchBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=None, dilation=1, groups=1,
                 internal_channels_1x1_3x3=None,deploy=False, single_init=False):
        super(DiverseBranchBlock, self).__init__()
        self.deploy = deploy

        self.nonlinear = Conv.default_act #成员属性
        ...
        if padding is None: #代码的维护性质强
            padding = autopad(kernel_size, padding, dilation)
        assert padding == kernel_size // 2

        if deploy: #部署
            self.dbb_reparam = nn.Conv2d(in_channels, out_channels, kernel_size, stride,padding, dilation, groups, bias=True)

        else:

            self.dbb_origin = conv_bn(in_channels, out_channels, kernel_size, stride, padding, dilation, groups=groups)

            self.dbb_avg = nn.Sequential()
            if groups < out_channels: #分组数小于输出通道数
                self.dbb_avg.add_module('conv',
                                        nn.Conv2d(in_channels, out_channels, 1,1,0, groups, bias=False)) #1x1
                self.dbb_avg.add_module('bn', BNAndPadLayer(padding, out_channels))
                self.dbb_avg.add_module('avg', nn.AvgPool2d(kernel_size, stride, padding=0))
                self.dbb_1x1 = conv_bn(in_channels, out_channels, 1, stride,0, groups)
            else:
            ...
            if internal_channels_1x1_3x3 == in_channels:
                ...
                self.dbb_1x1_kxk.add_module('conv1', nn.Conv2d(in_channels=in_channels, out_channels=internal_channels_1x1_3x3,
                                                            kernel_size=1, stride=1, padding=0, groups=groups, bias=False)) #i是否使用分组卷积
    ...
    def forward(self, inputs):
        ...
        out = self.dbb_origin(inputs) #torch.Size([16, 256, 80, 80]) 3x3卷积
        if hasattr(self, 'dbb_1x1'):
            out += self.dbb_1x1(inputs) #1x1卷积 逐元素相加
...
if __name__ == "__main__":
    x = torch.randn(16, 256, 80, 80)
    Diver1= DiverseBranchBlock(256,256,3)#输入通道 输出通道 卷积核大小
    out = Diver1(x) #torch.Size([16, 256, 80, 80])

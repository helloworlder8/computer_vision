...
class RTDETRDecoder(nn.Module):
    ...
    def __init__(
        ...
        # Denoising part
        self.denoising_class_embed = nn.Embedding(nc, hd) #类别 影藏层
    ...
    def forward(self, x, targets=None): #torch.Size([4, 256, 80, 80]) torch.Size([4, 256, 40, 40]) torch.Size([4, 256, 20, 20])
        ...
        dn_embed, dn_bbox, attn_mask, dn_meta = get_cdn_group(
            ...
            self.denoising_class_embed.weight, #torch.Size([256, 80])
            self.num_denoising, #num denoising噪声数 100
        ...
        dec_bboxes, dec_scores = self.decoder(
            ...
            self.query_pos_head,
            attn_mask=attn_mask,
        ) #torch.Size([6, 4, 498, 4]) torch.Size([6, 4, 498, 80]) torch.Size([4, 300, 4]) torch.Size([4, 300, 80])   {'dn_pos_idx': [torch.Size([36]), torch.Size([63]), torch.Size([99]), torch.Size([72])], 'dn_num_group': 9, 'dn_num_split': [198, 300]}
    ...
    def _get_encoder_input(self, x):#torch.Size([4, 256, 80, 80]) torch.Size([4, 256, 40, 40]) torch.Size([4, 256, 20, 20])
        """Processes and returns encoder inputs by getting projection features from input and concatenating them."""
        # Get projection features
        x = [self.input_proj[i](feat) for i, feat in enumerate(x)]#-》torch.Size([4, 256, 80, 80]) torch.Size([4, 256, 40, 40]) torch.Size([4, 256, 20, 20]) 线性变化
        # Get encoder inputs
        feats = []
        shapes = []
        for feat in x:
    ...
    def _get_decoder_input(self, feats, shapes, dn_embed=None, dn_bbox=None):
        ...
        features = self.enc_output(valid_mask * feats)  #torch.Size([4, 8400, 256])再经过一层线性变化

        enc_outputs_scores = self.enc_score_head(features)  # torch.Size([4, 8400, 80])

        # Query selection
        # (bs, num_queries)
        topk_ind = torch.topk(enc_outputs_scores.max(-1).values, self.num_queries, dim=1).indices.view(-1) #每张图片中取300个置信度最高的
        # (bs, num_queries)
        batch_ind = torch.arange(end=bs, dtype=topk_ind.dtype).unsqueeze(-1).repeat(1, self.num_queries).view(-1)

        # (bs, num_queries, 256)
        top_k_features = features[batch_ind, topk_ind].view(bs, self.num_queries, -1) #torch.Size([4, 300, 256])
        # (bs, num_queries, 4)
        ...
        embeddings = self.tgt_embed.weight.unsqueeze(0).repeat(bs, 1, 1) if self.learnt_init_query else top_k_features #torch.Size([4, 300, 256])
        if self.training:

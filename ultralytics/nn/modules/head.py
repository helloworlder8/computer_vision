...
class Detect(nn.Module):#检测头
    ...
    def decode_bboxes(self, bboxes, anchors): #torch.Size([2, 4, 3528]) torch.Size([1, 2, 3528])
...
# dn
    ...
    def forward(self, feats, gt=None):
        ...
        dn_cls_embed, dn_bboxs_embed, attn_mask, dn_meta = generate_cdn_train_sample(
            gt, #对目标进行处理
            self.nc, #80
            self.nq, #300
...
# combined_cls_embed, combined_bboxs, enc_bboxes, enc_cls topk_cls topk_bboxs
        combined_cls_embed, combined_bboxs, topk_bboxs, topk_cls = self._generate_combined_cls_bboxs(feats, shapes, dn_cls_embed, dn_bboxs_embed)

        # Decoder
        dec_bboxes, dec_cls = self.decoder(
            combined_cls_embed,
            combined_bboxs,
            feats,
            shapes,
    ...
    def _feats_converse(self, pd):#torch.Size([4, 256, 80, 80]) torch.Size([4, 256, 40, 40]) torch.Size([4, 256, 20, 20])
        """Processes and returns encoder inputs by getting projection features from input and concatenating them."""
        # Get projection features
        pd = [self.input_proj[i](feat) for i, feat in enumerate(pd)]#-》torch.Size([4, 256, 80, 80]) torch.Size([4, 256, 40, 40]) torch.Size([4, 256, 20, 20]) 线性变化
        # Get encoder inputs
        feats = []
        shapes = []
        for feat in pd:
            ...
            feats.append(feat.flatten(2).permute(0, 2, 1)) #批 点 通
            # [[80, 80], [40, 40], [20, 20]]
            shapes.append([h, w])

        # [b, h*w, c]
        feats = torch.cat(feats, 1) #torch.Size([4, 8400, 256])
    ...
    #     enc_cls = self.enc_score_head(features)  # torch.Size([4, 8400, 80])

    #     # Query selection
    #     # torch.Size([600])
    #     cls_topk_ind = torch.topk(enc_cls.max(-1).values, self.nq, dim=1).indices.view(-1) #每张图片中取300个置信度最高的
    #     # torch.Size([600])
    #     query_img_idx = torch.arange(end=bs, dtype=cls_topk_ind.dtype).unsqueeze(-1).repeat(1, self.nq).view(-1)

    #     # torch.Size([2, 300, 256])  每张图片查询300个模式
    #     cls_topk_feats = features[query_img_idx, cls_topk_ind].view(bs, self.nq, -1) 
    #     # torch.Size([2, 300, 4])
    #     topk_anchors = anchors[:, cls_topk_ind].view(bs, self.nq, -1)

    #     # Dynamic anchors + static content
    #     topk_bboxs = self.enc_bbox_head(cls_topk_feats) + topk_anchors
    #     # torch.Size([4, 300, 4])
    #     enc_bboxes = topk_bboxs.sigmoid()
    #     if dn_bboxs_embed is not None:
    #         combined_bboxs = torch.cat([dn_bboxs_embed, topk_bboxs], 1)
    #     enc_cls = enc_cls[query_img_idx, cls_topk_ind].view(bs, self.nq, -1)

    #     cls_topk_feats = self.tgt_embed.weight.unsqueeze(0).repeat(bs, 1, 1) if self.learnt_init_query else cls_topk_feats #torch.Size([2, 300, 256])
    #     if self.training:
    #         combined_bboxs = combined_bboxs.detach()
    #         if not self.learnt_init_query:
    #             cls_topk_feats = cls_topk_feats.detach()
    #     if dn_cls_embed is not None:
    #         combined_cls_embed = torch.cat([dn_cls_embed, cls_topk_feats], 1)
    #     # top_k加噪声  top_k加噪声 torch.Size([4, 300, 4]) torch.Size([4, 300, 80])
    #     return combined_cls_embed, combined_bboxs, enc_bboxes, enc_cls


    def _generate_combined_cls_bboxs(self, feats, shapes, dn_cls_embed=None, dn_bboxs_embed=None):
        """Generates and prepares the input required for the decoder from the provided feats_after_linear and shapes."""
        
        bs = feats.shape[0]
        
        # Prepare input for decoder
        anchors, valid_mask = self._generate_anchors(shapes, dtype=feats.dtype, device=feats.device)  # torch.Size([1, 8400, 4]) torch.Size([1, 8400, 1])
        feats_after_linear = self.enc_linear(valid_mask * feats)  # torch.Size([4, 8400, 256])再经过一层线性变化
        enc_cls = self.enc_score_head(feats_after_linear)  # torch.Size([4, 8400, 80])

        # Query selection - selecting top k based on confidence scores
        cls_topk_ind = torch.topk(enc_cls.max(-1).values, self.nq, dim=1).indices.view(-1)  # 每张图片中取300个置信度最高的
        query_img_idx = torch.arange(end=bs, dtype=cls_topk_ind.dtype).unsqueeze(-1).repeat(1, self.nq).view(-1)  # torch.Size([600])

        # Extract top-k feats_after_linear and anchors for each image
        cls_topk_feats = feats_after_linear[query_img_idx, cls_topk_ind].view(bs, self.nq, -1)  # torch.Size([2, 300, 256])
        topk_anchors = anchors[:, cls_topk_ind].view(bs, self.nq, -1)  # torch.Size([2, 300, 4])

        # Dynamic anchors + static content
        topk_bboxs = self.enc_bbox_head(cls_topk_feats) + topk_anchors  # torch.Size([4, 300, 4])
        topk_bboxs = topk_bboxs.sigmoid()  # torch.Size([4, 300, 4])

        # Combine dynamic and noise-augmented bbox embeddings if provided
        combined_bboxs = torch.cat([dn_bboxs_embed, topk_bboxs], 1) if dn_bboxs_embed is not None else topk_bboxs

        # Process classification scores
        topk_cls = enc_cls[query_img_idx, cls_topk_ind].view(bs, self.nq, -1)  # torch.Size([2, 300, 80])

        # Initialize or modify the query embedding for classification
        if self.learnt_init_query:
            cls_topk_feats = self.tgt_embed.weight.unsqueeze(0).repeat(bs, 1, 1)  # torch.Size([2, 300, 256])
        ...
        combined_cls_embed = torch.cat([dn_cls_embed, cls_topk_feats], 1) if dn_cls_embed is not None else cls_topk_feats

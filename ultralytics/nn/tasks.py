...
class RTDETRDetectionModel(DetectionModel):
    ...
    def loss(self, batch, preds=None):
        ...
        Returns:
            (tuple): A tuple containing the total loss and main three avg_loss_items in a tensor.
        """
        ...
        gt_groups = [(batch_idx == i).sum().item() for i in range(bs)] #[4, 7, 11, 8]
        targets = {
        ...
        if dn_meta is None:
            ...
            dn_bboxes, dec_bboxes = torch.split(dec_bboxes, dn_meta["dn_num_split"], dim=2) #torch.Size([6, 4, 198, 80]) torch.Size([6, 4, 300, 80])
    ...
    def predict(self, x, profile=False, visualize=False, targets=None, augment=False, embed=None):
        """
        Perform a forward pass through the model.

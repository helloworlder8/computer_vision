...
def draw_custom_marker(image, center, color, marker_type, size, edgecolor, thickness):
    x, y = center
    if marker_type == '*':
        # Draw a star shape
        cv2.drawMarker(image, (x, y), color, markerType=cv2.MARKER_STAR, markerSize=size, thickness=thickness, line_type=cv2.LINE_AA)
    elif marker_type == 'o':
        # Draw a circle with edge color
        cv2.circle(image, (x, y), size // 2, color, thickness=-1)  # Filled circle
        cv2.circle(image, (x, y), size // 2, edgecolor, thickness=thickness)  # Edge circle
    else:
        # Default to a cross if the marker type is not recognized
        cv2.drawMarker(image, (x, y), color, markerType=cv2.MARKER_CROSS, markerSize=size, thickness=thickness, line_type=cv2.LINE_AA)

# Main function to draw bounding boxes and optional center points with customization
def annotated_image_detection(
    image_folder, label_folder, output_folder, file_extension="JPG", palette=None, step_size=1, random_color_mode=False,
    draw_center_points=False, marker_type='*', marker_size=10, edgecolor=(255, 255, 255), thickness=1
):
    ...
    for image_path in image_files:
        ...
        for line in lines:
            ...
            if True:
                ...
                cv2.putText(image, class_names[class_id], (x1, y1), 0, 1, box_color, 1, lineType=cv2.LINE_AA) #字体 大小 粗细
                # Draw the center point with custom style if the option is enabled
                if draw_center_points:
                     draw_custom_marker(image, (int(x_center), int(y_center)), box_color, marker_type, marker_size, edgecolor, thickness)
...
if __name__ == "__main__":
    ...
    image_folder = 'datasets/roadrisk12/images/val2017/'
    label_folder = 'datasets/roadrisk12/labels/val2017/'
    output_folder = 'datasets/roadrisk12/vision/'
    file_extension = "jpg"  # Image file extension
    palette = None  # Use None for the default palette
    step_size = 1  # How many indices to jump in the palette

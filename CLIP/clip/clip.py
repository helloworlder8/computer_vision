...
def _transform(image):
    ...
    return Compose(
        [   #预处理是比较简单的
            Resize(image, interpolation=BICUBIC),
            CenterCrop(image),
...
def tokenize(
    ...
    start_token = _tokenizer.encoder["<|startoftext|>"] #49406
    end_token = _tokenizer.encoder["<|endoftext|>"] #49407
    all_tokens = [[start_token] + _tokenizer.encode(text) + [end_token] for text in texts] #[[49406, 320, 22697, 49407], [49406, 320, 1929, 49407], [49406, 320, 2368, 49407]]
    result = torch.zeros( #torch.Size([3, 77])
        len(all_tokens),
        context_length,
    ...
    return result #三句话 每一句话编码的向量长度（不超过77）list 2 torch
